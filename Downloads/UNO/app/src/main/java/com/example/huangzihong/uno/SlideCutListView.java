...
public class SlideCutListView extends ListView{
    ...
    //分发时间，主要做的事判断点击的是哪个item，以及通过postDelayed来设置响应左右滑动事件
    ...
    private void scrollRight(){
        ...
        //调用startScroll方法来设置一些滚动的参数，我们在computscroll（）方法中调用scrollTo来滚动item
    ...
    //根据手指滚动itemview的距离来判断是滚动到开始位置还是向左或者向右
    private void scrollByDistanceX(){
        //如果向左滚动的距离大于屏幕额分之一，就让其删除
    ...
    //获取X方向的滑动书u，大于0向右滑动，反之向左
    private int getScrollVelocity(){
        velocityTracker.computeCurrentVelocity(1000);
        int velocity=(int)velocityTracker.getXVelocity();
        return velocity;
    }
    //当listview item滑出屏幕，回调这个接口。我们需要在回调方法removeitem（）中移除该item，然然后刷新listview
